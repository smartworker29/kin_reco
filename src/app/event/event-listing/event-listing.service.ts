...
export class EventListingService {
  ...
  get_event_details(api_input) {
    if(api_input.event_date_start == undefined){
      api_input.event_date_start='';
    }
    let url = `${API_URL}` + 'events/?event_date_start='+ api_input.event_date_start + '&limit=90'+'&category=' + encodeURIComponent(api_input.category) +'&q=' + encodeURIComponent(api_input.q) +'&city=' + encodeURIComponent(api_input.city) +'&event_range_str=' + encodeURIComponent(api_input.event_range_str)+ '&order_by=date_dist_asc';
   if(api_input.distance){
   url = `${API_URL}` + 'events/?event_date_start='+ api_input.event_date_start + '&limit=90'+'&category=' + encodeURIComponent(api_input.category) +'&q=' + encodeURIComponent(api_input.q) +'&city=' + encodeURIComponent(api_input.city) +'&event_range_str=' + encodeURIComponent(api_input.event_range_str) +'&order_by=date_dist_asc'+'&distance=' + encodeURIComponent(api_input.distance)
   }
   if(api_input.kid_id){
